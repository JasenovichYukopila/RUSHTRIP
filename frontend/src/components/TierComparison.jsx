...
const TIERS = [
  { key: 'economico', label: 'Económico', stars: '1-3★', icon: '💪', basePct: 0.65 },
  { key: 'estandar', label: 'Estándar', stars: '3-4★', icon: '⭐', basePct: 0.85 },
  { key: 'premium', label: 'Premium', stars: '4-5★', icon: '👑', basePct: 1.15 },
];
        ...
        {TIERS.map((tier) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-2">
                {tier.key === 'economico' && dentro && pct < 60 && (
                  <span className="badge bg-success/15 text-success border border-success/20 text-[10px]">✅ Dentro</span>
                )}
                {tier.key === 'estandar' && dentro && (
                  <span className="badge bg-accent/15 text-accent border border-accent/20 text-[10px]">⭐ Recomendado</span>
                )}
                {tier.key === 'estandar' && !dentro && (
                  <span className="badge bg-warning/15 text-warning border border-warning/20 text-[10px]">⚠️ Excede</span>
                )}
                {tier.key === 'premium' && dentro && (
                  <span className="badge bg-accent2/15 text-accent2-600 border border-accent2/20 text-[10px]">💎 Disponible</span>
                )}
                {tier.key === 'premium' && !dentro && (
                  <span className="badge bg-warning/15 text-warning border border-warning/20 text-[10px]">💎 Sobre presupuesto</span>
                )}
              </div>
            </div>
          );
